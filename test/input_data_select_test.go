...
func TestJq1(t *testing.T) {
	...
	l1, _ := core.Select(jqExpression, &inputData)
...
func TestJq2(t *testing.T) {
	...
	l1, _ := core.Select(jqExpression1, &inputData)
	json1, _ := json.Marshal(l1)
	t.Log(string(json1))
	l2, _ := core.Select(jqExpression2, &inputData)
	json2, _ := json.Marshal(l2)
	t.Log(string(json2))
	l3, _ := core.Select(jqExpression3, &inputData)
	json3, _ := json.Marshal(l3)
	t.Log(string(json3))
	l4, _ := core.Select(jqExpression4, &inputData)
