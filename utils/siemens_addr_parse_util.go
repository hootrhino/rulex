...
*
 */
func ParseDB(s string) (AddressInfo, error) {
	AddressInfo := AddressInfo{}
	if len(s) < 3 {
		return AddressInfo, fmt.Errorf("invalid Address format:%s", s)
	}
	if s[:2] == "I." {
		return ParseADDR_I(s)
	}
	if s[:2] == "Q." {
		return ParseADDR_Q(s)
	}
	if s[:2] == "IB" {
		return ParseADDR_IB(s)
	}
	if s[:2] == "QB" {
		return ParseADDR_QB(s)
	}
	if s[:2] == "DB" {
		return __ParseDB_DX(s)
	}
...
// 解析DB: DB4900.DBD2108
// ^DB\d+\.\w+\.\d+$
func __ParseDB_DX(s string) (AddressInfo, error) {
	AddressInfo := AddressInfo{}
	Len := len(s)
	if Len < 3 {
		return AddressInfo, fmt.Errorf("invalid Address format:%s", s)
	...
	if s[:2] == "DB" {
		...
		ElementNumber, err2 := strconv.Atoi(parts[1][4:])
		if err2 != nil {
			return AddressInfo, fmt.Errorf("ElementNumber %s Atoi failed:%s", s, err2)
		}
		// DBD: 4字节
		if parts[1][2] == 'D' {
			AddressInfo.DataBlockType = "WORD"
		}
		// DBX: 1字节
		if parts[1][2] == 'X' {
...
func ParseADDR_I(s string) (AddressInfo, error) {
	AddressInfo := AddressInfo{}
	if len(s) < 2 {
		return AddressInfo, fmt.Errorf("invalid Address format:%s", s)
	}
...
// IB0
func ParseADDR_IB(s string) (AddressInfo, error) {
	AddressInfo := AddressInfo{}
	AddressInfo.AddressType = "IB"
	AddressInfo.DataBlockNumber = 0
	AddressInfo.BitNumber = 0
	return AddressInfo, nil
}
...
func ParseADDR_Q(s string) (AddressInfo, error) {
	AddressInfo := AddressInfo{}
	if len(s) < 2 {
		return AddressInfo, fmt.Errorf("invalid Address format:%s", s)
	}
...
func ParseADDR_QB(s string) (AddressInfo, error) {
	AddressInfo := AddressInfo{}
